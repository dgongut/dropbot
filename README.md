...
| FFMPEG_HW                      | ❌           | Encoder hardware para convertir vídeos: `NONE` (por defecto, `libx264`), `VAAPI` (Intel/AMD, recomendado), `NVENC` (NVIDIA) o `QSV` (Intel Quick Sync, experimental: necesita el runtime de Intel y no viene en la imagen). Si el encoder hardware falla, se reintenta automáticamente con `libx264` |
